...
class F1L12_1_Generator_Q3_F_MQ extends QuestionGenerator_1.QuestionGenerator {
    ...
    generateLevel5() {
        ...
        return [
            {
                coefficient: coef1,
                variables: new Map(vars.map((v, i) => [v, exponents[i].exp1]))
            },
            {
                coefficient: coef2,
                variables: new Map(vars.map((v, i) => [v, exponents[i].exp2]))
            },
            {
                coefficient: coef3,
                variables: new Map(vars.map((v, i) => [v, exponents[i].exp3]))
            }
        ];
    }
    shuffleArray(array) {
    ...
    formatQuestion(terms) {
        // 計算結果
        const result = {
            coefficient: terms.reduce((acc, term, index) => index === 0 ? term.coefficient : acc / term.coefficient, terms[0].coefficient),
            variables: new Map()
        };
        // 計算每個變量的最終指數
...
3. 最終答案：\\(${answer}\\)`;
    ...
    generateWrongAnswers(correctAnswer, difficulty) {
    ...
    formatTerm(coefficient, variables) {
        if (variables.size === 0)
...
exports.default = F1L12_1_Generator_Q3_F_MQ;
