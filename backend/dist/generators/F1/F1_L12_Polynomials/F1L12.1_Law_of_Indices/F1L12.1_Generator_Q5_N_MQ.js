...
const FractionUtils_1 = require("../../../../utils/FractionUtils");
class F1L12_1_Generator_Q5_N_MQ extends QuestionGenerator_1.QuestionGenerator {
    constructor(difficulty = 1) {
        super(difficulty, 'F1L12.1');
        this.VARIABLES = ['x', 'y', 'z', 'a', 'b', 'm', 'n', 'p', 'q', 'r'];
        this.EASY_COEFFICIENTS = [2, 3, 4, 6, 8, 9, 12, 15, 16, 18, 20];
    }
    generate() {
        let terms = [];
    ...
    generateLevel5() {
        ...
        return [
            {
                coefficient: coef1,
                variables: new Map(vars.map(v => [v, Math.floor(Math.random() * 3) + 2])),
                operation: 'multiply'
            },
            {
                coefficient: coef2,
            ...
            {
                coefficient: coef3,
                variables: new Map(vars.map(v => [v, Math.floor(Math.random() * 2) + 1])),
                operation: 'divide'
            }
        ];
    }
    formatQuestion(terms) {
        // 计算结果，使用分数运算
        let resultCoefficient = [terms[0].coefficient, 1];
        // 从第二项开始计算，每个数都作为分数处理
        for (let i = 1; i < terms.length; i++) {
            const term = terms[i];
            if (term.operation === 'divide') {
                resultCoefficient = FractionUtils_1.FractionUtils.divide(resultCoefficient, [term.coefficient, 1]);
            }
            else {
                resultCoefficient = FractionUtils_1.FractionUtils.multiply(resultCoefficient, [term.coefficient, 1]);
            }
        }
        const result = {
            // 保持分数形式，不要转换为小数
...
1. 係數運算：
   \\[${terms.reduce((str, term, index) => {
        ...
        }, '')} = ${FractionUtils_1.FractionUtils.toLatex(resultCoefficient[0], resultCoefficient[1])}\\]
...
3. 最終答案：\\[${answer}\\]`;
    ...
    generateWrongAnswers(correctAnswer, terms) {
        const wrongAnswers = new Set();
        // 收集题目中出现的所有变量
        const questionVars = new Set();
        terms.forEach(term => {
        ...
        if (this.difficulty <= 3) {
            ...
            const sameCoeffAnswer = (isCorrectAnswerFraction ?
                FractionUtils_1.FractionUtils.toLatex(correctNum, correctDen) :
                correctNum.toString()) +
            ...
            if (isCorrectAnswerFraction) {
                [differentNum, differentDen] = FractionUtils_1.FractionUtils.simplify(correctNum * 3, correctDen * 2);
            }
            else {
            ...
            const differentCoeffStr = isCorrectAnswerFraction ?
                FractionUtils_1.FractionUtils.toLatex(differentNum, differentDen) :
    ...
    formatTerm(term) {
        ...
        if (term.coefficient !== 1) {
            ...
            if (term.coefficient % 1 !== 0) {
                ...
                const [simplifiedNum, simplifiedDen] = FractionUtils_1.FractionUtils.simplify(numerator, denominator);
                coefficientStr = FractionUtils_1.FractionUtils.toLatex(simplifiedNum, simplifiedDen);
            }
            else {
    ...
    // 新增方法：使用分数形式格式化项
    formatTermWithFraction(term, coefficient) {
        let coefficientStr = '';
        if (term.coefficient !== 1) {
            // 直接使用分数形式
            coefficientStr = FractionUtils_1.FractionUtils.toLatex(coefficient[0], coefficient[1]);
        }
        if (term.variables.size === 0)
            return coefficientStr || '1';
        const sortedVars = Array.from(term.variables.entries())
...
exports.default = F1L12_1_Generator_Q5_N_MQ;
