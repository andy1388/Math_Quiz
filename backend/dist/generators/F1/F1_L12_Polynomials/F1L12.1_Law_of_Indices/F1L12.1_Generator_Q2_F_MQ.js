...
class F1L12_1_Generator_Q2_F_MQ extends QuestionGenerator_1.QuestionGenerator {
    constructor(difficulty = 1) {
        super(difficulty, 'F1L12.1');
        this.VARIABLES = ['x', 'y', 'z', 'a', 'b', 'm', 'n', 'p', 'q', 'r'];
        this.missingExponent = 0; // 記錄缺失的指數值
    }
    generate() {
        let terms = [];
        let result;
    ...
    generateLevel5() {
        ...
        const result = {
            ...
            variables: new Map(vars.map((v, i) => [v, exponents[i] * 2]))
        };
        return [
            [
                {
                    coefficient: coef1,
                    variables: new Map(vars.map((v, i) => [v, exponents[i]]))
                },
                {
                    coefficient: coef2,
                    variables: new Map(vars.map((v, i) => [v, exponents[i]])),
...
exports.default = F1L12_1_Generator_Q2_F_MQ;
