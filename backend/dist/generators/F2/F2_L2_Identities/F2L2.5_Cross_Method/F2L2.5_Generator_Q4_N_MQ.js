...
class F2L2_5_Generator_Q4_N_MQ extends QuestionGenerator_1.QuestionGenerator {
    constructor(difficulty = 1) {
        super(difficulty, 'F2L2.5');
        // 定义不同难度可用的变量
        this.basicVariables = ['x', 'y'];
        this.advancedVariablePairs = [
            ['m', 'n'], ['p', 'q'], ['r', 's'], ['u', 'v']
        ];
    }
    gcd(a, b) {
    ...
    getRandomVariables() {
        ...
        // 难度4-5随机选择一对变量
        return this.advancedVariablePairs[Math.floor(Math.random() * this.advancedVariablePairs.length)];
    }
    generate() {
        // 生成因子
    ...
    calculateExpandedCoefficients(factors) {
        const { e } = factors;
        if (this.difficulty <= 2) {
            return [1, 1]; // 不需要分数处理
        }
        if (this.difficulty === 3 || this.difficulty === 4) {
            return [e, 1]; // 整数系数（包括负数）
        }
        // 难度5：分数系数
        return e; // 返回分子分母
    }
    formatExpression(p, q, factors) {
        const { a, b, c, variables: [var1, var2] } = factors;
        // 计算展开式的系数
        const x2Coeff = (p * a) / q; // x²的系数
        const xyCoeff = (p * (b + c)) / q; // xy的系数
        const y2Coeff = (p * b * c) / q; // y²的系数
        // 辅助函数：检查是否需要使用分数表示（超过2位小数）
        const needsFraction = (num) => {
        ...
        if (this.difficulty <= 2) {
            ...
            if (this.difficulty === 3) {
                ...
                wrongChoices = [
                    { ...factors, e: [p + 1, q] },
                    { ...factors, e: [p, q + 1] },
                    { ...factors, e: [q, p] } // 分子分母互换
                ];
            }
            // 再添加一些改变括号内容的选项
            wrongChoices.push({ ...factors, a: a + 1, c: c - 1 }, { ...factors, a: a - 1, c: c + 1 }, { ...factors, b: -b, d: -d });
        }
        // 打乱错误选项顺序
...
exports.default = F2L2_5_Generator_Q4_N_MQ;
